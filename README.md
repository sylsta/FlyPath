...
3. If the RC appears as a removable/lettered drive or SD card (not as a USB MTP device), click **Locate folder manually** and browse to the `waypoint` folder. (A USB-connected RC is an MTP device and will not appear in this dialog — use Auto Detect RC for that.)
